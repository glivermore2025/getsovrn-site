...
export default function Marketplace() {
  ...
  useEffect(() => {
    if (activeTab === 'buy') filterAndSort();
          ...
          {sellLoading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
              {buyerPosts.map((post) => (
                <div
                  key={post.id}
                  className="bg-gray-900 p-6 rounded-xl shadow hover:shadow-green-700/30 transition"
                >
                  <h3 className="text-2xl font-semibold mb-2 text-white">
                    {post.title}
                  </h3>
                  ...
                  <div className="flex justify-center gap-2 mt-4">
                    <Link href={`/buyer_posts/${post.id}`}>
                      <span className="flex-1 text-center bg-green-600 hover:bg-green-700 px-4 py-2 rounded-lg text-sm font-medium cursor-pointer">
                        View Request
                      </span>
                    </Link>
                  </div>
